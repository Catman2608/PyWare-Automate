...
class App(CTk):
    def __init__(self):
        ...
        self.recording_file = os.path.join(USER_CONFIG_DIR, "recording.txt")
    ...
    def load_recording_file(self):
        ...
        Structure:
            ...
            configs/ConfigName/recording.txt
        """
        ...
        txt_path = os.path.join(rod_folder, "recording.txt")
        ...
        if os.path.exists(ahk_path):
            ...
            # If txt file doesn't exist → create empty
        ...
        try:
            ...
            self.recorded_actions = []
    def load_recording_from_txt(self):
        """Load recorded actions from a text file. If it does not exist, create it."""
        try:
            # If recording file does not exist → create it as an empty file
            if not os.path.exists(self.recording_file):
                with open(self.recording_file, "w") as f:
                    f.write("")  # create empty file
                self.recorded_actions = []
                print("No recording file found. Created empty recording.txt")
                return

            # If exists → load normally
            with open(self.recording_file, "r") as f:
                self.recorded_actions = [line.strip() for line in f.readlines()]
            print("Recording loaded:", len(self.recorded_actions), "actions")

        except Exception as e:
            print("Error loading recording (2):", e)
    # Macro functions
    ...
    def _normalize_key_for_ahk(self, key):
        ...
        if hasattr(key, "char") and key.char:
            ...
            # special symbols require braces in AHK
            return f"{{{c}}}"
    ...
    def playback_action(self, action, speed=1.0):
        ...
        if action.startswith("SetBatchLines") or action.startswith(";") or action.startswith("F5::") or action.startswith("F7::") or action.startswith("ExitApp") or action.lower() == "return":
            return  # ignore this AHK performance command
    ...
    def start_playback(self):
        ...
        while self.macro_running and (infinite_loop or loop_count < loops):
            ...
            for action in self.recorded_actions:
                ...
                try:
                    self.playback_action(action, speed)
                except Exception as e:
