...
class App(CTk):
    ...
    def build_general_tab(self, parent):
        ...
        start_playback_key_entry = CTkEntry( hotkey_settings, width=120, textvariable=start_playback_key_var )
        ...
        stop_playback_key_entry = CTkEntry( hotkey_settings, width=120, textvariable=stop_playback_key_var )
        ...
        playback_options.grid(row=2, column=1, padx=20, pady=20, sticky="nw")

        CTkLabel(playback_options, text="Record and Playback", font=CTkFont(size=14, weight="bold")).grid(row=0, column=0, padx=12, pady=8, sticky="w")

        CTkLabel(playback_options, text="Record Delay").grid(row=1, column=0, padx=12, pady=8, sticky="w") # This is the label syntax
        ...
        record_delay_entry = CTkEntry(playback_options, width=120, textvariable=record_delay_var) # This line initializes the entry
        ...
        CTkLabel(playback_options, text="Playback Loops").grid(row=2, column=0, padx=12, pady=8, sticky="w") # This is the label syntax
        ...
        playback_loops_entry.grid(row=2, column=1, padx=12, pady=8, sticky="w") # This line initializes the position for the entry (most important)

        CTkLabel(playback_options, text="Playback Interval (minutes)").grid(row=3, column=0, padx=12, pady=8, sticky="w") # This is the label syntax
        ...
        playback_interval_entry.grid(row=3, column=1, padx=12, pady=8, sticky="w") # This line initializes the position for the entry (most important)

        CTkLabel(playback_options, text="Playback speed (minutes)").grid(row=4, column=0, padx=12, pady=8, sticky="w") # This is the label syntax
        ...
        playback_speed_entry.grid(row=4, column=1, padx=12, pady=8, sticky="w") # This line initializes the position for the entry (most important)
    ...
    def load_recording_file(self):
        ...
        # Try to load AHK file if it exists
        if os.path.exists(ahk_path):
            path = ahk_path
        else:
            print("File not found! Record to start generating AutoHotKey files.")
    ...
    def _normalize_key_for_ahk(self, key):
        ...
        # --- Fallback: convert <63> (decimal) => sc3f (hex) for AHK SC lookup ---
        s = str(key)

        if s.startswith("<") and s.endswith(">"):
            code = s[1:-1]
            try:
                # pynput gives decimal codes; AHK SC map uses hex, so convert
                hex_code = format(int(code), "x")
                sc_key = f"sc{hex_code}"
                if sc_key in SC_TO_KEY:
                    return SC_TO_KEY[sc_key]  # resolve to real key name, e.g. "f5"
                return sc_key
            except ValueError:
    ...
    def on_key_press_record(self, key):
        ...
        # Use record_action so elapsed time is captured as a Sleep line
    ...
    def start_recording(self):
        ...
        while self.macro_running:
            ...
            if now - self.last_record_time >= scan_delay:
                # Log last mouse position if moved (mouse moves are still buffered)
